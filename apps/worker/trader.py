...
class PaperTrader:
    ...
    def open_position(
        ...
        if sys_settings.position_size_mode == "PERCENTAGE":
            ...
            # Risk a percentage of wallet per trade
            # Assume stop_loss_pct represents the max risk taken.
            # amount_usd * (stop_loss_pct/100) = current_balance * (risk_per_trade/100)
            ...
            if self.wallet.total_trades < 10:
                ...
                loss_trades = self.wallet.total_trades - win_trades
            ...
            kelly_fraction = max(0.01, min(0.99, kelly_fraction)) # cap between 1% and 99%
    ...
    def check_position(self, position: Position, token: dict, sys_settings: SystemSettings) -> Optional[Position]:
        ...
        if position.trailing_stop_price and current_price <= position.trailing_stop_price:
            execution_price = max(current_price, position.trailing_stop_price * (1 - (sys_settings.paper_slippage_pct/100.0)))
            return self.close_position(position, execution_price, sys_settings, exit_reason=f"Trailing Stop Hit")
            
        # 3. Hard Stop Loss Hit
        if current_price <= position.stop_loss:
            execution_price = max(current_price, position.stop_loss * (1 - (sys_settings.paper_slippage_pct/100.0)))
            return self.close_position(position, execution_price, sys_settings, exit_reason=f"Hard Stop Loss Hit")
